...
public class TwitterInputAdapter extends AdapterApp {
    ...
    private LinkedBlockingQueue<String> messageQueue = new LinkedBlockingQueue<String>(400000);
...
//    }
    ...
    class ProduceZipf implements Runnable {
        ...
        public void run() {
            ...
            while (true) {
                try {
                	Timer.Context rateCntxt = rateTimer.time();
                	rateLimiter.acquire();
                	rateCntxt.stop();
                	Timer.Context timerContext = produceTimer.time();
                    String tweet = "This is test #topic" + getZipfSample(zd, sampleList) + " generated by apache zipf distribution.";
                    messageQueue.put(tweet);
                    srcSuccMeter.mark();
                    timerContext.stop();
                } catch (Exception e) {
                    srcFailMeter.mark();
    ...
    class Dequeuer implements Runnable {
        ...
        private final Timer dequeTimer = getMetricRegistry().timer(MetricRegistry.name("event-dequeue-cost"));
        
        @Override
        public void run() {
            while (true) {

            	Timer.Context timerContext = dequeTimer.time();
                try {
